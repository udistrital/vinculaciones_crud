...
type Vinculacion struct {
	...
	TerceroPrincipalId     int              `orm:"column(tercero_principal_id)"`
	TerceroRelacionadoId   int              `orm:"column(tercero_relacionado_id);null"`
	TipoVinculacionId      *TipoVinculacion `orm:"column(tipo_vinculacion_id);rel(fk)"`
	CargoId                *Cargos          `orm:"column(cargo_id);rel(fk)"`
...
func GetAllVinculacion(query map[string]string, fields []string, sortby []string, order []string,
	...
	o := orm.NewOrm()
	qs := o.QueryTable(new(Vinculacion))
	// query k=v
	for k, v := range query {
